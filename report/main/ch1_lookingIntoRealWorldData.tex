...
\end{figure}
\paragraph{}
The curves are skewed to the right (it is particularly obvious in the case of the Sanofi stock). It corresponds to the case $\gamma > 0$ in the generalised extreme value distribution representation $G_\gamma$ or, equivalently, to a Fréchet-type distribution.
\clearpage
...
\end{figure}
\paragraph{}
The data-above-threshold plots for the Carrefour and Total stocks show that the data seems to fit a Gumbel distribution (corresponds to the case $\gamma = 0$ in the generalised extreme value distribution representation $G_\gamma$).
\clearpage
...
\begin{figure}[h!]
	...
	\caption{Density fitting for the LVMH data - 97.5 \% quantile \\}
	\label{fig:dataAboveThresholdLVMHFitted}
\end{figure}
\paragraph{}
As we suspected the structure of the above the 95 \%-quantile data for the LVMH stock makes the fitting arduous. To say the least, it is hard to recognize an extreme value distribution. If we take the data above the 97.5 \%-quantile, however, the shape of the curve is that of a generalised extreme value distribution with $\gamma < 0$ that is a Weibull-type distribution.