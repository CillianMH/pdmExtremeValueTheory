...
It is customary to model the evolution of stock prices by a stochastic process $(S_t)_{t \ge 0}$ satisfying the Black-Scholes stochastic differential equation, which reads as follows : 
\begin{equation}
...
\phi_0 = 1
\end{array}
\right.$ \\ Solving this ODE is elementary and yields the solution $\phi_t = \exp(\mu t)$. Now, let us differentiate $X_t$ under its form as a product  $S_t = \phi_t Z_t$. \\
\begin{equation}
\begin{alignat*}{2}
d(S_t) &= d(\phi_t Z_t)\\ 
&= \phi_t dZ_t + Z_t d\phi_t + d<\phi,Z>_t\\ 
&= \phi_t dZ_t + \mu \phi_t Z_t dt\\
&= \phi_t dZ_t + \mu S_t dt\\
\end{alignat*}
\end{equation} where the infinitesimal quadratic covariation between $\phi$ and $Z$ is zero as $\phi$ has bounded variations. If we compare the last right-hand term of the series of equations just above to the original Black-Scholes SDE, we see that $\sigma S_t dB_t = \phi_t dZ_t$. Hence, $dZ_t = \sigma Z_t dB_t$. Here, we must be very careful as this differential equation does not integrate as it would in the settings of 'usual' calculus, in particular, integrating it to $\log(Z_t) - \log(Z_0) = \sigma (B_t - B_0)$ is \underline{totally wrong !}
\paragraph{Resolution - step 2}
Now, let us set $Y_t = \log(Z_t)$. Using Ito-Doeblin's formula, we get : \\
$d(\log(Z_t)) = \frac{1}{Z_t} dZ_t + \frac{1}{2} (\frac{-1}{Z^2_{t}})d<Z>_t$ ($\star$)\\
\underline{$\rightarrow$ How to compute $d<Z>_t$ ?}
\begin{equation}
\begin{alignat*}{2}
S_t &= \phi_t Z_t\\ 
\implies Z_t &= \frac{S_t}{\phi_t}\\ 
\implies dZ_t &= \frac{1}{\phi_t} dS_t + S_t d(\frac{1}{\phi_t}) + d<\frac{1}{\phi},S>_t\\
&= \frac{1}{\phi_t} dS_t - \frac{S_t}{\phi_t^2} d(\phi_t) \\
&= \frac{1}{\phi_t} dS_t - \frac{\mu S_t}{\phi_t} dt \\
&= \frac{1}{\phi_t} (\mu S_t dt + \sigma S_t dB_t) - \frac{\mu S_t}{\phi_t} dt \\
&= \frac{\sigma S_t}{\phi_t} dB_t 
\end{alignat*}
...
\implies d<Z>_t &= \sigma^2 Z_t^2 dt
\end{alignat*}
\end{equation} \newline
Back to ($\star$), we now have :
\begin{equation}
\begin{alignat*}{2}
d(\log(Z_t)) &= \frac{1}{Z_t} dZ_t + \frac{1}{2} (\frac{-1}{Z^2_{t}}) \sigma^2 Z^2_{t} dt \\
\iff \frac{1}{Z_t} dZ_t &= d(\log(Z_t)) + \frac{\sigma^2}{2} dt 
\end{alignat*}
\end{equation} 
\paragraph{Resolution - step 3}
Combining the previous equation with $dZ_t = \sigma Z_t dB_t$, we get : \newline
\begin{equation}
\begin{alignat*}{2}
\sigma dB_t &= d(log(Z_t)) + \frac{\sigma^2}{2} dt \\
...
Let us set $Y_t = \log(S_t)$, we then have $dY_t = \frac{1}{S_t}dS_t -\frac{1}{2} \frac{1}{S_t^2} d<S>_t$. If we remember that $dS_t = \mu(t) S_t dt + \sigma(t) S_t dB_t$ and apply the Isometry formula, we get that $d<S>_t = \sigma(t)^2 X_t^2 dt$. We thus get : \newline
\begin{equation}
\begin{alignat*}{2}
\sigma dY_t &= \frac{1}{S_t} dS_t - \frac{1}{2} \frac{1}{S_t^2} d<S>_t \\
&=  \frac{1}{S_t} dS_t - \frac{1}{2} \sigma(t)^2 dt \\
&=  \frac{1}{S_t} (\mu(t) S_t dt + \sigma(t) S_t dB_t)- \frac{1}{2} \sigma(t)^2 dt \\
 &= (\mu(t) - \frac{1}{2} \sigma(t)^2) dt + \sigma(t) dBt \\
